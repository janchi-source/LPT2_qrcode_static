...
//   POST /api/override    { id, postup, heslo }            ručná oprava
//   POST /api/simulacia   { kod, heslo }                   prežene jeden QR celou hrou
//   POST /api/reset       { potvrdenie: 'ZMAZAT', heslo }
//
// Skenovať smie ktokoľvek — telefón má na stanovišti v ruke ktorýkoľvek
// animátor a zdržiavať ho prihlasovaním by hru len brzdilo. Zásahy, ktoré
// menia stav mimo pravidiel hry (override, simulácia, reset), sú za heslom.
const hra = require('./hra');
const db = require('./ulozisko');

// Spoločné heslo pre zásahy do hry. Dá sa prepísať premennou prostredia,
// nech sa nemusí meniť kód. Nie je to bezpečnostná hranica pred útočníkom —
// je to poistka proti omylu a proti zvedavému dieťaťu s telefónom.
const HESLO = process.env.LPT2S_HESLO || 'sex';

function hesloSedi(telo) {
  return typeof telo.heslo === 'string' && telo.heslo.trim() === HESLO;
}
...
async function handler(req, res) {
  ...
  try {
    ...
    if (cesta === 'override' && req.method === 'POST') {
      if (!hesloSedi(telo)) return json(res, 401, { error: 'Nesprávne heslo.', treba_heslo: true });
    ...
    if (cesta === 'simulacia' && req.method === 'POST') {
      // Za heslom rovnako ako override — posunie dieťa cez celú hru, čo je
      // presne to, čo sa nemá dať spraviť omylom ani zo žartu.
      if (!hesloSedi(telo)) return json(res, 401, { error: 'Nesprávne heslo.', treba_heslo: true });
      const dieta = hra.najdiDieta(telo.kod);
      if (!dieta) return json(res, 400, { error: 'Dieťa sa nenašlo.' });
      const kroky = [];
